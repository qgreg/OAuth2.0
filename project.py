...
engine = create_engine('postgres://luqjhiocljctha:KD42kXmgujk7EVETt2VYp33oP0@ec2-107-21-219-142.compute-1.amazonaws.com:5432/d3kalufc5sjllt')
...
def editRestaurant(restaurant_id):
  ...
  if login_session['user_id'] != editedRestaurant.user_id:
    flash('User does not have permission to edit %s.' % editedRestaurant.name)
...
def deleteRestaurant(restaurant_id):
  ...
  if login_session['user_id'] != restaurantToDelete.user_id:
    flash('User does not have permission to delete %s.' % restaurantToDelete.name)
    return redirect('/restaurant')
  if request.method == 'POST':
...
def newMenuItem(restaurant_id):
  ...
  if login_session['user_id'] != restaurant.user_id:
    flash('User does not have permission to create a new menu item for %s.' % restaurant.name)
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if login_session['user_id'] != editedItem.user_id:
      flash('User does not have permission to edit the menu item %s.' % editedItem.name)
      return redirect(url_for('showMenu', restaurant_id = restaurant_id))
...
def deleteMenuItem(restaurant_id,menu_id):
    ...
    if login_session['user_id'] != itemToDelete.user_id:
      flash('User does not have permission to delete the menu item %s.' % itemToDelete.name)
      return redirect(url_for('showMenu', restaurant_id = restaurant_id))
    if request.method == 'POST':
